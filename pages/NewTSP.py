...
for i in range(int(n_cities)):
    ...
    x_coord = st.number_input(f"Enter X coordinate for {city_name}", value=float(random.randint(1, 20)))
    y_coord = st.number_input(f"Enter Y coordinate for {city_name}", value=float(random.randint(1, 20)))
    city_coords[city_name] = (x_coord, y_coord)

# Proceed with the algorithm only if cities are entered
if len(city_coords) == n_cities:
